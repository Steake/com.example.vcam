...
- **Injected in-app UI** (the interesting one): `InjectedConfigActivity`, a `Theme.VCAM.Translucent` activity exposed under the intent action `com.example.vcam.action.CONFIGURE`. A hook — or the user, or a shortcut — launches it to re-pick, swap, or clear media, toggle between *global* and *per-app* behaviour, and dismiss back to the caller. Picker failures in constrained contexts are caught and surfaced rather than crashing the target app.
...
stateDiagram-v2
    ...
    [*] --> LaunchedFromHook: ACTION_CONFIGURE

    LaunchedFromManager --> Dialog
    LaunchedFromHook --> Dialog
...
6. **Inside the target app**, either launch `InjectedConfigActivity` directly (your choice of launcher shortcut, `am start -a com.example.vcam.action.CONFIGURE`, or the manager's overflow) to swap media without context-switching back to the manager.
